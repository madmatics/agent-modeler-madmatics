...
from madmatics.mmm.base import BaseValidateMMM
from madmatics.mmm.causal import CausalGraphModel
...
from madmatics.mmm.fourier import YearlyFourier
from madmatics.mmm.hsgp import SoftPlusHSGP
from madmatics.mmm.lift_test import (
    add_lift_measurements_to_likelihood_from_saturation,
    scale_lift_measurements,
)
from madmatics.mmm.preprocessing import MaxAbsScaleChannels, MaxAbsScaleTarget
from madmatics.mmm.tvp import create_time_varying_gp_multiplier, infer_time_index
from madmatics.mmm.utility import UtilityFunctionType, average_response
from madmatics.mmm.utils import (
    apply_sklearn_transformer_across_dim,
    create_new_spend_data,
)
from madmatics.mmm.validating import ValidateControlColumns
from madmatics.model_builder import _handle_deprecate_pred_argument
from madmatics.model_config import parse_model_config
from madmatics.model_graph import deterministics_to_flat
...
class BaseMMM(BaseValidateMMM):
    ...
    def build_model(
        ...
        **kwargs : dict
        ...
        t1_control_columns: list[str] = []
        non_t1_control_columns: list[str] = []
        if self.control_columns:
            t1_control_columns = [col for col in self.control_columns if "t1" in col]
            non_t1_control_columns = [
                col for col in self.control_columns if col not in t1_control_columns
            ]

            if t1_control_columns:
                self.model_coords["control_t1"] = t1_control_columns
            if non_t1_control_columns:
                self.model_coords["control_no_t1"] = non_t1_control_columns
        with pm.Model(coords=self.model_coords) as self.model:
            ...
            if self.time_varying_intercept:
                intercept_baseline = self.model_config["intercept"].create_variable("intercept_baseline")
                ...
                intercept = self.model_config["intercept"].create_variable("intercept")

            t1_contributions_sum = 0
            remaining_control_contributions_sum = 0

            # control 전체 처리 (t1 포함 분리)
            if self.control_columns and all(col in self.preprocessed_data["X"].columns for col in self.control_columns):
                ...
                if isinstance(self.model_config["gamma_control"], dict):
                    gammas = []
                    for col in self.control_columns:
                        prior = self.model_config["gamma_control"][col]
                        if not hasattr(prior, "dims") or prior.dims != ():
                            prior.dims = ()
                        # ✅ 내부 이름은 raw로 명확히 지정
                        gamma_i = prior.create_variable(name=f"gamma_control_{col}_raw")
                        gammas.append(gamma_i)

                    gamma_control_raw = pt.stack(gammas)

                else:
                    if self.model_config["gamma_control"].dims != ("control",):
                        self.model_config["gamma_control"].dims = "control"
                    gamma_control_raw = self.model_config["gamma_control"].create_variable(name="gamma_control_raw")

                # ✅ 공통: 외부용 이름으로 Deterministic 선언
                gamma_control = pm.Deterministic("gamma_control", gamma_control_raw, dims=("control",))


                # 전체 control 기여도 계산
                full_control_contributions = pm.Deterministic(
                    name="full_control_contributions",
                    var=control_data * gamma_control,
                    dims=("date", "control"),
                )

                t1_indices = [
                    i for i, col in enumerate(self.control_columns) if col in t1_control_columns
                ]
                non_t1_indices = [
                    i for i, col in enumerate(self.control_columns) if col in non_t1_control_columns
                ]

                if t1_indices:
                    t1_contributions = pm.Deterministic(
                        name="t1_contributions",
                        var=full_control_contributions[:, t1_indices],
                        dims=("date", "control_t1"),
                    )
                    t1_contributions_sum = pm.Deterministic(
                        name="t1_contributions_sum",
                        var=t1_contributions.sum(axis=-1),
                        dims="date",
                    )
                ...
                if non_t1_indices:
                    control_contributions = pm.Deterministic(
                        ...
                        var=full_control_contributions[:, non_t1_indices],
                        dims=("date", "control_no_t1"),
                    )
                    remaining_control_contributions_sum = control_contributions.sum(axis=-1)

            intercept = pm.Deterministic(
                name="intercept_base",
                var=intercept + t1_contributions_sum,
                dims="date",
            )
            ...
            mu_var = intercept + remaining_control_contributions_sum + channel_contributions.sum(axis=-1)

            if self.yearly_seasonality is not None:
                dayofyear = pm.Data(
                    "dayofyear",
                    self.preprocessed_data["X"][self.date_column].dt.dayofyear.to_numpy(),
                    dims="date",
                )

                def create_deterministic(x):  # for plotting
                    pm.Deterministic("fourier_contributions", x, dims=("date", *self.yearly_fourier.prior.dims))

                yearly_seasonality_contribution = pm.Deterministic(
                    "yearly_seasonality_contribution",
                    self.yearly_fourier.apply(dayofyear, result_callback=create_deterministic),
                    dims="date",
                )
...
class MMM(
    ...
    def optimize_budget(
        ...
        **minimize_kwargs
        ...
        """
        from madmatics.mmm.budget_optimizer import BudgetOptimizer
