...
from madmatics.mmm.validating import (
    ValidateChannelColumns,
    ValidateDateColumn,
    ValidateTargetColumn,
)
from madmatics.model_builder import ModelBuilder
...
class MMMModelBuilder(ModelBuilder):
    ...
    def compute_mean_contributions_over_time(
        ...
        """Get the contributions of each channel over time, including trend if available."""
        ...
        if getattr(self, "control_columns", None):
            ...
            # ✅ 추가: trend_contributions가 있다면 control_contributions에 합치기
            if "trend_contribution" in self.fit_result:
                contributions_trend_over_time = (
                    az.extract(
                        self.fit_result,
                        var_names=["trend_contribution"],
                        combined=True,
                    )
                    .mean("sample")
                    .to_dataframe()
                ...
                # trend는 하나의 시계열일 가능성이 높음 → 열이 1개일 수 있음
                if isinstance(contributions_trend_over_time, pd.Series):
                    contributions_trend_over_time = contributions_trend_over_time.to_frame(name="trend")

                contributions_control_over_time = pd.concat(
                    [contributions_control_over_time, contributions_trend_over_time],
                    axis=1,
                )
        else:
        ...
        if original_scale:
            ...
            all_contributions_over_time.columns = all_contributions_over_time.columns.map(
    ...
    def plot_waterfall_segments_decomposition(
        self,
        original_scale: bool = True,
        figsize: tuple[int, int] = (14, 7),
        **kwargs,
    ) -> plt.Figure:
        """Waterfall plot with grouped 'channel' and 'control' contributions."""

        # 1. 전체 contributions 데이터
        df = self.compute_mean_contributions_over_time(original_scale=original_scale)

        # 2. 'channel' 그룹 합산
        grouped = {
            "channel": df[self.channel_columns].sum(axis=1),
        }

        # 3. 'control' 그룹 합산 (trend 포함 시)
        control_cols = [col for col in self.control_columns if col in df.columns] if self.control_columns else []
        if "trend" in df.columns:
            control_cols.append("trend")

        if control_cols:
            grouped["control"] = df[control_cols].sum(axis=1)

        # 4. 나머지 개별 항목 유지
        for col in df.columns:
            if col not in self.channel_columns and col not in control_cols:
                grouped[col] = df[col]

        # 5. DataFrame → 평균 기여도 계산
        contrib_df = pd.DataFrame(grouped)
        mean_contrib = contrib_df.mean().sort_values(ascending=False)

        total = mean_contrib.sum()
        percent = 100 * mean_contrib / total

        # 6. 시각화
        fig, ax = plt.subplots(figsize=figsize, layout="constrained", **kwargs)
        cumulative = 0

        for idx, (name, value) in enumerate(mean_contrib.items()):
            bar_start = cumulative if value >= 0 else cumulative + value
            ax.barh(
                name,
                value,
                left=bar_start,
                color="royalblue" if value >= 0 else "indianred",
                alpha=0.5,
            )
            label_pos = bar_start + value / 2
            ax.text(
                label_pos,
                name,
                f"{value:,.0f}\n({percent[name]:.1f}%)",
        ...
        xticks = np.linspace(0, total, num=11)
        ax.set_xticks(xticks)
        ax.set_xticklabels([f"{x / total * 100:.0f}%" for x in xticks])

        ax.spines["top"].set_visible(False)
        ax.spines["right"].set_visible(False)
        ax.spines["left"].set_visible(False)
        ax.set_yticks(range(len(mean_contrib)))
        ax.set_yticklabels(mean_contrib.index)
