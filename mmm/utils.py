...
def apply_sklearn_transformer_across_dim(
    ...
    # 🔒 안전한 차원 추가 (기존에 "_" 있으면 생략)
    if "_" not in data.dims:
        data = data.expand_dims("_")

    # 🔍 core dims 사전 검사
    if dim_name not in data.dims:
        raise ValueError(f"Expected dim '{dim_name}' not found in data.dims: {data.dims}")
    ...
    # 💡 squeeze 제거 시 원래 shape 유지됨
    if "_" in transformed.dims:
        transformed = transformed.squeeze("_")

    # 🧼 차원 순서 복원 (문제 방지용 try)
    try:
        transformed = transformed.transpose(*original_dims)
    except ValueError as e:
        print(f"[WARNING] transpose failed: {e} — using original order.")
