...
from madmatics.deserialize import deserialize
from madmatics.hsgp_kwargs import HSGPKwargs
from madmatics.prior import Prior, VariableFactory
...
def parse_model_config(
    ...
    def handle_prior_config(name, prior_config):
        ...
        # ✨ 추가: dict 형태의 Prior들 처리
        if isinstance(prior_config, dict):
            try:
                return {
                    k: v if isinstance(v, (Prior, VariableFactory)) else deserialize(v)
                    for k, v in prior_config.items()
                }
            except Exception as e:
                parse_errors.append(f"Parameter {name} (as dict): {e}")
                return prior_config
