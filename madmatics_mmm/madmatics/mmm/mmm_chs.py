        ...
        **kwargs : dict
        ...
        with pm.Model(coords=self.model_coords) as self.model:
            ...
            if (
                ...
                and all(column in self.preprocessed_data["X"].columns for column in self.control_columns)
            ):
                if isinstance(self.model_config["gamma_control"], dict):
                    ...
                    for col in self.control_columns:
                        ...
                        if not hasattr(prior, "dims") or prior.dims != ():
                            prior.dims = ()
                        gamma_i = prior.create_variable(name=f"gamma_control_{col}")
                        gammas.append(gamma_i)

                    gamma_control = pt.stack(gammas)
                    gamma_control = pm.Deterministic("gamma_control", gamma_control, dims=("control",))

                    control_contributions = pm.Deterministic(
                        "control_contributions",
                        control_data * gamma_control,
                        dims=("date", "control"),
                    )
                    ...
                    gamma_control = self.model_config["gamma_control"].create_variable(name="gamma_control")
            ...
            if self.yearly_seasonality is not None:
                dayofyear = pm.Data(
                    name="dayofyear",
                    value=self.preprocessed_data["X"][self.date_column].dt.dayofyear.to_numpy(),
                    dims="date",
                )
                ...
                chs_basline = pm.Deterministic(
                    name="chs_basline",
                    var=yearly_seasonality_contribution + channel_contributions.sum(axis=-1),
                    dims="date",
                )
