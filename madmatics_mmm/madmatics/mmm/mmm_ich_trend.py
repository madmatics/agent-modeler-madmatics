...
class BaseMMM(BaseValidateMMM):
    ...
    def build_model(
        ...
        **kwargs : dict
        ...
        # 👇 coords에 새 dim 추가
        if "trend" in self.control_columns:
            non_trend_columns = [col for col in self.control_columns if col != "trend"]
            self.model_coords["control_no_trend"] = non_trend_columns
        with pm.Model(coords=self.model_coords) as self.model:
            ...
            control_contributions_sum = 0
            trend_contribution = 0

            # control 전체 처리 (trend 포함)
            if self.control_columns and all(col in self.preprocessed_data["X"].columns for col in self.control_columns):
                ...
                # trend 분리 저장
                if "trend" in self.control_columns:
                    trend_idx = self.control_columns.index("trend")

                    trend_contribution = pm.Deterministic(
                        name="trend_contribution",
                        var=full_control_contributions[:, trend_idx],
                        dims="date",
                    )

                    non_trend_indices = [i for i, col in enumerate(self.control_columns) if col != "trend"]

                    control_contributions = pm.Deterministic(
                        name="control_contributions",
                        var=full_control_contributions[:, non_trend_indices],
                        dims=("date", "control_no_trend"),
                    )
                    control_contributions_sum = control_contributions.sum(axis=-1)

                else:
                    control_contributions = pm.Deterministic(
                        name="control_contributions",
                        var=full_control_contributions,
                        dims=("date", "control"),
                    )
                    control_contributions_sum = control_contributions.sum(axis=-1)
            ...
            intercept = pm.Deterministic(
                name="intercept_base",
                var=intercept + channel_contributions.sum(axis=-1),
                dims="date",
            )

            mu_var = intercept + control_contributions_sum
            ...
            if self.yearly_seasonality is not None:
                ...
                if trend_contribution != 0:
                    yt_contribution = pm.Deterministic(
                        "yt_contribution",
                        yearly_seasonality_contribution + trend_contribution,
                        dims='date'
                    )

                    mu_var += yt_contribution
                else:
